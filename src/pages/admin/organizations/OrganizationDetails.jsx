...
const OrganizationDetails = () => {
    ...
    if (loading) {
        return (
            <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '50vh', color: 'var(--neutral-500)' }}>
        ...
        return (
            <div style={{ padding: '24px' }}>
                ...
                <button
                    ...
                    style={{ marginTop: '16px', padding: '8px 16px', background: 'var(--surface-color)', border: '1px solid var(--neutral-200)', borderRadius: '6px', cursor: 'pointer' }}
                >
    ...
    return (
        <div style={{ padding: '24px' }}>
            <div style={{ marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '16px' }}>
                ...
                <div>
                    <h1 style={{ fontSize: '24px', fontWeight: 'bold', margin: '0 0 4px 0', color: 'var(--neutral-900)', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        {organization.name}
                        <span style={{ padding: '4px 10px', background: 'var(--primary-50)', color: 'var(--primary-600)', fontSize: '12px', fontWeight: 600, borderRadius: '9999px', verticalAlign: 'middle' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: '24px' }}>
                ...
                <div style={{ background: 'var(--surface-color)', borderRadius: '8px', border: '1px solid var(--neutral-200)', overflow: 'hidden' }}>
                    <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--neutral-200)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--neutral-50)' }}>
                        <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 600, color: 'var(--neutral-900)', display: 'flex', alignItems: 'center', gap: '8px' }}>
                            <Building2 size={18} style={{ color: 'var(--neutral-500)' }} />
                            Organization Info
                        </h3>
                        <div style={{ display: 'flex', gap: '8px' }}>
                            <button
                                ...
                                style={{ padding: '6px 12px', background: 'var(--surface-color)', border: '1px solid var(--neutral-200)', borderRadius: '6px', fontSize: '13px', fontWeight: 500, cursor: isActionsDisabled ? 'not-allowed' : 'pointer', color: isActionsDisabled ? 'var(--neutral-400)' : 'var(--primary-600)', display: 'flex', alignItems: 'center', gap: '6px', opacity: isActionsDisabled ? 0.6 : 1 }}
                            >
                            ...
                            <button
                                ...
                                disabled={isActionsDisabled}
                                style={{ padding: '6px 12px', background: 'var(--surface-color)', border: '1px solid var(--neutral-200)', borderRadius: '6px', fontSize: '13px', fontWeight: 500, cursor: isActionsDisabled ? 'not-allowed' : 'pointer', color: isActionsDisabled ? 'var(--neutral-400)' : 'var(--neutral-900)', display: 'flex', alignItems: 'center', gap: '6px', opacity: isActionsDisabled ? 0.6 : 1 }}
                            >
                                <Edit2 size={14} /> Edit
                            </button>
                        </div>
                    ...
                    <div style={{ padding: '20px' }}>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '12px', marginBottom: '12px', fontSize: '14px' }}>
                            <div style={{ color: 'var(--neutral-500)', fontWeight: 500 }}>Global ID</div>
                            <div style={{ color: 'var(--neutral-900)', fontFamily: 'monospace' }}>{organization.id}</div>
                        </div>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '12px', marginBottom: '12px', fontSize: '14px' }}>
                            <div style={{ color: 'var(--neutral-500)', fontWeight: 500 }}>Name</div>
                            <div style={{ color: 'var(--neutral-900)', fontWeight: 500 }}>{organization.name}</div>
                        </div>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '12px', marginBottom: '12px', fontSize: '14px' }}>
                            <div style={{ color: 'var(--neutral-500)', fontWeight: 500 }}>Address</div>
                            <div style={{ color: 'var(--neutral-900)' }}>{organization.address || organization.organization_address || organization.settings?.address || '—'}</div>
                        </div>
                    </div>
                </div>
                ...
                <div style={{ background: 'var(--surface-color)', borderRadius: '8px', border: '1px solid var(--neutral-200)', overflow: 'hidden' }}>
                    <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--neutral-200)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--neutral-50)' }}>
                        <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 600, color: 'var(--neutral-900)', display: 'flex', alignItems: 'center', gap: '8px' }}>
                            <User size={18} style={{ color: 'var(--neutral-500)' }} />
                            Primary Admin Info
                        </h3>
                        <div style={{ display: 'flex', gap: '8px' }}>
                            <button
                    ...
                    <div style={{ padding: '20px' }}>
                        {adminUser && adminUser.email ? (
                            <>
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '12px', marginBottom: '12px', fontSize: '14px' }}>
                                    <div style={{ color: 'var(--neutral-500)', fontWeight: 500 }}>Name</div>
                                    <div style={{ color: 'var(--neutral-900)', fontWeight: 500 }}>{`${adminUser.first_name || ''} ${adminUser.last_name || ''}`}</div>
                                </div>
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '12px', marginBottom: '12px', fontSize: '14px' }}>
                                    <div style={{ color: 'var(--neutral-500)', fontWeight: 500 }}>Email</div>
                                    <div style={{ color: 'var(--neutral-900)' }}>{adminUser.email}</div>
                                </div>
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '12px', marginBottom: '12px', fontSize: '14px' }}>
                                    <div style={{ color: 'var(--neutral-500)', fontWeight: 500 }}>Mobile</div>
                                    <div style={{ color: 'var(--neutral-900)' }}>{adminUser.phone || '—'}</div>
                                </div>
                            </>
                        ) : (
                            <div style={{ color: 'var(--neutral-500)', fontSize: '14px', fontStyle: 'italic', padding: '12px 0' }}>
